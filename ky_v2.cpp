...
const long max_number_of_coefficients=(1UL<<16);
  ...
  */
  ...
  do
      ...
      if((T.pmv[ct_atoms].mantissa()==NTL::ZZ(0)) && (T.pmv[ct_atoms].exponent()==0))
	{
      ...
      if((unsigned long)(-ee)>=max_number_of_coefficients)
	{
	  std::cerr << "std::bitset length not big enough --- exit";
	  std::cerr << "\n" << -ee << "  " << max_number_of_coefficients << "\n";
	  exit(-1);
...
int main(void)
{
  ...
  */
  ...
  long sample_size=100000;
  long nb_chks;//number of highest probabilities to check
  //We check if their corresponding frequencies are close.
  if(P.nb_atoms<20)//If support size < 10 say, then check all of them.
    {
      nb_chks=P.nb_atoms;
    }
  else
  ...
  simple_freq_check(P, sample_size, nb_chks);
