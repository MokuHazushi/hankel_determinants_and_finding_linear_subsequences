...
std::mutex square_of_zeros_filling_mutex;

void assign_GF2(NTL::Mat<NTL::GF2> & mat, const NTL::GF2 & val, int j, int i)
{
  if (MULTI_THREAD_ON)
...
void init_experiment(struct experiment & experiment)
{
  ...
  experiment.AspTL[0][0][0]=NTL::GF2(0);//*fast*, initialized however never needed
  ...
  for(int i2=0;i2<n;i2++)//j=0,1
    {
      ...
      experiment.flags_M[0][i2]=true;//*fast, flagged as completed
	  ...
	  if( (j>=2) && (experiment.M[j-2][i]==NTL::GF2(1)) )//North-South-East-West
	    {
	      ...
	      //assign_GF2(experiment.M,experiment.M[j-1][i-1]*experiment.M[j-1][i+1]+experiment.M[j-1][i],j,i);
	  ...
	  else if ( (j>=2*max_len_side_grid) && chk_conds_for_solvability(experiment, j, i, effective_len) )
	    {
	      ...
	      //assign_GF2(experiment.M,solve_eq_for_lower_corner(experiment, j, i, effective_len),j,i);
	      experiment.flags_M[j][i] = true;
	    }
	  else
	    {
...
void solve_j_trivial(struct experiment & experiment, int j, int start, int range)
{
  ...
  for(int i = start; i<start+range; i++)//pour single thread est equivalent a i=j-1 ... i < j-1+n-2*j+2=n-j+1
      ...
      for( int r = 0; r<j ; r++)
	{
      ...
      //assign_GF2(experiment.M,NTL::determinant(tmp),j,i);//this does not work, but problem_fixing_mutex just after works
...
int main(void)
{
  ...
  std::cout << "\n\n***** ***** ***** ***** ***** ***** ***** ***** ***** *****\n"
	    ...
	    << "\n\n" << ct_problems << " mismatche(s) out of " << nb_trials << " random test cases\n";

  std::cout << "\nAverages of time differences between methods." << "\nOne average is over the sample of size " << nb_trials << ".\n"; 
